    ...
    public function execute(?RestDtoInterface $restDto, EntityInterface $entity, string $transactionId): void
    {
        ...
        if (isset($plainPassword) &&
            ...
            if (str_starts_with($plainPassword, 'interno://')) {
                ...
                $encoder = $this->userPasswordEncoder->getEncoder($this->tokenStorage->getToken()->getUser());

                if (!$encoder->isPasswordValid(
                    $this->tokenStorage->getToken()->getUser()->getPassword(),
                    $plainPassword,
                    $this->tokenStorage->getToken()->getUser()->getSalt()
                )) {
                    throw new RuntimeException('Senha não confere!');
                }
            } else if (str_starts_with($plainPassword, 'ldap://')) {
                try {
            ...
            } else if (str_starts_with($plainPassword, 'govBr://')) {
                $tokenRevalidaGovBr = str_replace('govBr://', '', $plainPassword);
                $cpfUsuario = $this->tokenStorage->getToken()->getUser()->getUsername();
            ...
            if ('A1' === $restDto->getAssinatura()) {
                ...
                if ('dev' === $this->parameterBag->get('kernel.environment')) {
                    $params = [
                        'java',
                        '-jar',
                        '/app/docker/java/supp-signer-1.8.jar',
                        '--mode=test',
                        '--hash='.$hash,
                    ];
                } else {
