...
class RateListenerRequest implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(
        ...
        $resourceLimiter = $this->perResourceLimiter->create(
            $this->tokenStorage->getToken()->getUser()->getUsername().'_'.$resource
        );
        ...
        if (!$isAPI) {
            // per user
            $userLimiter = $this->perUserLimiter->create($this->tokenStorage->getToken()->getUser()->getUsername());
            ...
            // per api
            $apiLimiter = $this->perApiLimiter->create($this->tokenStorage->getToken()->getUser()->getUsername());
