...
class JWTDecodedSubscriber implements EventSubscriberInterface
{
    ...
    private function checkPayload(JWTDecodedEvent $event, ?Request $request = null): void
    {
        ...
        if ((!isset($payload['passwordExpired']) || $payload['passwordExpired'] === true)
            && !($this->tokenStorage->getToken()
                && $this->tokenStorage->getToken()->hasAttribute('ignore_expired_password')
                && $this->tokenStorage->getToken()->hasAttribute('ignore_expired_password') === true)) {
            $event->markAsInvalid();
        }

        /** @var Usuario $usuario */
        $usuario = $this->usuarioRepository->loadUserByUsername($payload['username']);
