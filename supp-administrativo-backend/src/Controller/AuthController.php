...
class AuthController extends Controller
{
    ...
    public function recoverPasswordAction(
        ...
        $message = (new Swift_Message('Recuperação de senha no '.$parameterBag
                ->get('supp_core.administrativo_backend.nome_sistema')))
            ->setFrom($parameterBag->get('supp_core.administrativo_backend.email_suporte'))
    ...
    public function alterarSenhaAction(
        ...
        try {
            ...
            $preAuthToken = new PreAuthenticationJWTUserToken($request->get('token'));
            $preAuthToken->setAttribute('ignore_expired_password', true);
            $tokenStorage->setToken($preAuthToken);
            $payload = $jwtManager->decode($preAuthToken);
            ...
            if ($payload['authProviderKey'] !== 'login') {
                throw new ValidatorException(
                    'Alteração de senha disponível apenas para autenticação com usuário e senha.'
                );
            }

            $usuario = $usuarioResource->getRepository()->loadUserByUsername($payload[$idClaim]);
            ...
            $token = new UsernamePasswordToken($usuario, $payload['authProviderKey'], $usuario->getRoles());
            $tokenStorage->setToken($token);
            $event = new InteractiveLoginEvent($request, $token);
            $eventDispatcher->dispatch($event, 'security.interactive_login');
